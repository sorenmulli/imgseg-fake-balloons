...
from imgseg02561.utils import get_data_loader, get_optimizer, get_scheduler, train_one_epoch, get_model, evaluate
...
def downstream(model, args: JobDescription):
    ...
    dataset_val = get_coco(args.data_path, "val", NotImplementedError)
...
if __name__ == "__main__":
    ...
    parser.document()
    for job in jobs:
        log.configure(os.path.join(job.location, f"training.log"), print_level=LogLevels.DEBUG)
        log.log_repo()
